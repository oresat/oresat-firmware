...
bool mmc5883maI2CWriteRegister2(I2CDriver *i2cp, const uint8_t reg_number, const uint8_t value) {
    ...
    if( i2cMasterTransmitTimeout(i2cp, MMC5883MA_I2C_ADDRESS, tx, 2, NULL, 0, MMC5883MA_DEFAULT_I2C_TIMEOUT) == MSG_OK ) {
    	return(true);
    }
    return(false);
}
...
bool mmc5883maI2CReadRegister2(I2CDriver *i2cp, const uint8_t reg_number, uint8_t *dest_value) {
    ...
    if( i2cMasterTransmitTimeout(i2cp, (i2caddr_t) MMC5883MA_I2C_ADDRESS, tx, 1, rx, 1, MMC5883MA_DEFAULT_I2C_TIMEOUT) == MSG_OK ) {
    	*dest_value = rx[0];
    	return(true);
    }
    return(false);
...
bool mmc5883maReadData(MMC5883MADriver *devp, mmc5883ma_data_t *dest) {
    ...
    msg_t r = i2cMasterTransmitTimeout(devp->config->i2cp, MMC5883MA_I2C_ADDRESS, &reg, 1, rx, 6, MMC5883MA_DEFAULT_I2C_TIMEOUT);
