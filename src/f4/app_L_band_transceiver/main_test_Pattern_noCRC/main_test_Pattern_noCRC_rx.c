...
ax5043_regval_t reg_values[] = {
  ...
  {AX5043_REG_PKTMAXLEN,           0xFC,common},  // Rx max packet length (Tx length + 3)
...
ax5043_confval_t conf_values[]={
  ...
  {AXRADIO_PHY_PREAMBLE_BYTE           ,0xAA},  // was 55
...
static void main_loop(void)
{
    ...
    uint8_t axradio_rxbuffer1[256];  //buffer to receive radio data
    ...
    while (true)
    {
        ...
        palWaitLineTimeout(LINE_SX_INT0, TIME_MS2I(10000));
        ...
        packet_len=receive_loop(&axd1, axradio_rxbuffer, axradio_rxbuffer1);
        ...
        if(packet_len > 0)
        {
            // chprintf(DEBUG_CHP,"INFO: Packet Received %d\r\n",axradio_rxbuffer[3]);
            chprintf(DEBUG_CHP,"INFO: Packet Length %d\r\n",packet_len);
            chprintf(DEBUG_CHP,"INFO: Packet Data\r\n");
            for (uint8_t i = 0; i < 240; ++i)
            {
                chprintf(DEBUG_CHP,"    [%d] = 0x%02X\r\n", i, axradio_rxbuffer[i]);
            }
            for (uint8_t i = 0; i < packet_len; ++i)
            {
                chprintf(DEBUG_CHP,"    [%d] = 0x%02X\r\n", i+239, axradio_rxbuffer1[i]);
            }
        }
    }
